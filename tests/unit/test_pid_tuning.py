...
class TestCoordinateDescentTuner:
    def test_converges_on_convex_cost(self) -> None:
        # 既知の凸コスト: 最適点 (kp,ki,kd) = (2.0, 1.0, 0.5)
        def cost(g: PIDGains) -> float:
            return (g.kp - 2.0) ** 2 + (g.ki - 1.0) ** 2 + (g.kd - 0.5) ** 2

        tuner = CoordinateDescentTuner(PIDGains(kp=1.0, ki=0.1, kd=0.0), max_runs=200)
    ...
    def test_stops_at_max_runs(self) -> None:
        tuner = CoordinateDescentTuner(PIDGains(kp=1.0, ki=0.1, kd=0.0), max_runs=5)
        count = 0
        while (cand := tuner.next_candidate()) is not None:
    ...
    def test_keeps_best_gains(self) -> None:
        tuner = CoordinateDescentTuner(PIDGains(kp=1.0, ki=0.1, kd=0.0), max_runs=20)
        # 最初の候補（ベースライン）だけ良コスト、以降は悪コスト
        first = True
        while (cand := tuner.next_candidate()) is not None:
            tuner.report(cand, 0.0 if first else 10.0)
            first = False
        assert tuner.best == PIDGains(kp=1.0, ki=0.1, kd=0.0)
