...
**実機走行テスト結果**:
- シャシダイナモ稼働時の車速受信を確認
- 約3.86 km/h から 160.00 km/h（上限値）まで約4秒で加速するデータを正常受信
- 160.00 km/h でクランプ後、緩やかな減速（160→156 km/h）も正常デコード
- 約20ms間隔（50Hz）でフレームを安定受信
- `sudo .venv/bin/python scripts/check_can.py` にて動作確認完了 ✅
