...
### OE（Output Enable）によるハードウェア非常停止の冗長化（16ch 本実装時に検討）

現行の非常停止設計（`docs/architecture.md` L282-284）は GPIO17 割り込み→ソフトウェア経由（`ButtonServoDriver.release_all()` 等）でサーボを待機位置へ戻す方式であり、**ソフトウェアが正常に動作していることが前提**。PCA9685 の `OE` 端子（負論理、LOW=出力有効／HIGH=出力無効）を Raspberry Pi の GPIO に接続しておくと、I2C 通信やソフト処理を待たずに**物理的に即座に全16chのPWM出力を止められる**ため、ソフトフリーズ時の最終防衛線として有効。

**配線案（1個の疎通確認スクリプトでは不要、16ch 本実装時に追加）**:

| 接続元（Raspberry Pi 5） | 物理ピン | 接続先（PCA9685） | 備考 |
|--------------------------|----------|-------------------|------|
| GPIO22                   | 物理ピン 15 | OE              | 出力可否の一括制御（負論理） |

- ピン選定理由: 既存の GPIO 割り当て（GPIO2/3=I2C、GPIO17=非常停止スイッチ、GPIO27=予約/未使用）と重複しない空きピンのため GPIO22 を採用。
- ロジックレベル: PCA9685 の `VCC`（ロジック電源）は本設計で Pi の 3.3V を使用しているため、OE のしきい値も 3.3V 系と一致し、レベルシフタなしで GPIO22 を直結できる。
- ソフト制御: 起動時に GPIO22 を出力に設定し **LOW（有効）** を維持。既存の非常停止トリガ（GPIO17 割り込みハンドラ）から `GPIO22 = HIGH` を追加で駆動し、ソフト経由の `release_all()` と並行してハード的にも出力を即遮断する。
- フェイルセーフ: PCA9685 ブレイクアウト基板は OE に内蔵プルダウンを持つものが多く、Pi 起動前後で GPIO22 が未駆動（フローティング）でも既定は LOW=出力有効側に倒れる。誤って常時無効側に倒したくない場合は基板のプルダウン仕様を確認すること。
- この配線・制御は Post-MVP の `ButtonServoDriver` 本実装（16ch）に合わせて追加するものであり、本ステアリング（1個の SG90 疎通確認）のスコープには含めない。
